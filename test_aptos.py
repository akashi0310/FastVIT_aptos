"""
Test / Inference script for FastViT on the APTOS 2019 test set.
Generates a submission.csv file using a trained checkpoint.

This script supports loading the custom model defined in this repository,
or the official FastViT model from the timm library.
It automatically handles both standard and reparameterized (fused) checkpoints.

Usage on Kaggle:
    python test_aptos.py \
        --checkpoint "/kaggle/working/output_fastvit/bestVIT.pth" \
...
def main():
    parser = argparse.ArgumentParser(description="Inference on APTOS 2019 Test Set using FastViT")
    # Paths
    parser.add_argument("--checkpoint", required=True, help="Path to bestVIT.pth or best_reparam.pth")
    ...
    parser.add_argument("--use-timm", action="store_true", help="Whether the checkpoint was trained on timm model")
    parser.add_argument("--model-timm", default="fastvit_t8", help="Timm model name if using --use-timm")
    parser.add_argument("--img-size", type=int, default=256, help="Image size for inference (FastViT default is 256)")
    # Execution
    ...
    if not os.path.exists(checkpoint_path):
        ...
        for root, dirs, files in os.walk("/kaggle/input"):
            for f in files:
                if f.endswith(".pth") and ("best" in f.lower() or "vit" in f.lower() or "fast" in f.lower()):
    ...
    is_reparam = checkpoint.get("reparam", False)
    use_timm = ckpt_args.get("use_timm", args.use_timm)
    model_timm = ckpt_args.get("model_timm", args.model_timm)
    variant = ckpt_args.get("variant", args.variant)

    print("\nModel configuration:")
    print(f"  - Library: {'timm' if use_timm else 'custom model.py'}")
    print(f"  - Model variant/name: {model_timm if use_timm else variant}")
    print(f"  - Number of classes: {num_classes}")
    print(f"  - Image size: {img_size}")
    print(f"  - Is reparameterized: {is_reparam}")
    ...
    if use_timm:
        import timm
        print(f"Creating timm model: {model_timm}")
        model = timm.create_model(model_timm, pretrained=False, num_classes=num_classes)
        if is_reparam and hasattr(model, "switch_to_deploy"):
            model.switch_to_deploy()
    else:
        print(f"Creating custom model (inference_mode={is_reparam}): {variant}")
        model = fast_vit(num_classes=num_classes, inference_mode=is_reparam, variant=variant)
    ...
    # Reparameterize on-the-fly if not already reparameterized
    if not is_reparam:
        print("Model is currently in training mode structure. Reparameterizing on-the-fly for faster testing...")
        if use_timm:
            if hasattr(model, "switch_to_deploy"):
                model.switch_to_deploy()
                print("Successfully reparameterized timm model.")
        else:
            model = reparameterize_model(model)
            print("Successfully reparameterized custom model.")
