"""
Fine-tune a FastViT backbone on the APTOS 2019 dataset (classification).

This script supports training the custom model defined in this repository (from scratch),
or fine-tuning the official FastViT model from the timm library using pretrained weights.

To train the custom model:
    python train_aptos.py --data-dir "/path/to/aptos2019" --variant T8

To train using timm with pretrained weights:
    python train_aptos.py --data-dir "/path/to/aptos2019" --use-timm --model-timm fastvit_t8 --pretrained
"""
import argparse
import os
import sys
...
def build_datasets(args, train_tf, val_tf):
    ...
    raise SystemExit(
        ...
        "Pass explicit --train-csv/--train-img-dir (or --train-dir/--val-dir)."
    )
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune FastViT on APTOS 2019")
    # Data
    ...
    # Model
    parser.add_argument("--variant", default="T8", choices=["T8", "T12", "S12", "SA12", "SA24", "SA36", "MA36"],
                        help="FastViT variant from model.py (ignored if using --use-timm)")
    parser.add_argument("--use-timm", action="store_true", help="Use official timm library implementation")
    parser.add_argument("--model-timm", default="fastvit_t8", help="Timm model name if using --use-timm")
    parser.add_argument("--pretrained", action="store_true", help="Load ImageNet pretrained backbone (only for timm)")
    parser.add_argument("--num-classes", type=int, default=5, help="Number of output classes (APTOS has 5)")
    parser.add_argument("--img-size", type=int, default=256, help="FastViT default training size is 256")
    ...
    if args.use_timm:
        import timm
        print(f"Initializing FastViT model from timm: {args.model_timm} (pretrained={args.pretrained})")
        ...
        print(f"Initializing custom FastViT model from model.py: {args.variant} (no pretrained weights)")
        model = fast_vit(
            num_classes=args.num_classes,
    ...
    if os.path.exists(ckpt_path):
        ...
        if args.use_timm:
            import timm
            best_model = timm.create_model(args.model_timm, pretrained=False, num_classes=args.num_classes)
        else:
            best_model = fast_vit(num_classes=args.num_classes, inference_mode=False, variant=args.variant)
        ...
        if args.use_timm:
            # Check if timm FastViT model supports deploy/reparam fusion
            if hasattr(best_model, "switch_to_deploy"):
                best_model.switch_to_deploy()
                print("Successfully reparameterized timm model using switch_to_deploy().")
            else:
                print("Note: Timm model does not expose switch_to_deploy() directly. Saved as standard model.")
        else:
            best_model = reparameterize_model(best_model)
            print("Successfully reparameterized custom model using reparameterize_model().")

        # Save fused model state dict
        reparam_ckpt = os.path.join(args.output, "best_reparam.pth")
        torch.save(
            {"state_dict": best_model.state_dict(), "acc": checkpoint["acc"], "args": checkpoint["args"], "reparam": True},
            reparam_ckpt
        )
        print(f"Saved reparameterized model to: {reparam_ckpt}")
        print("---------------------------------------------------------------------------\n")
